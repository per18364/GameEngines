...
Uint32 fragment(Uint32 currentColor, STexture *skin)
{
    Uint8 r = (currentColor >> 16) & 0xFF;
    Uint8 g = (currentColor >> 8) & 0xFF;
    Uint8 b = currentColor & 0xFF;
    ...
    std::cout << "r: " << (int)r << " g: " << (int)g << std::endl;

    Uint32 skinnedColor = skin->getPixel(r, g);
    std::cout << "skinnedColor: " << (int)skinnedColor << std::endl;
...
void Game::setup()
{
    ...
    STexture *skin = new STexture(renderer, window);
    skin->load("./src/Colores.png");

    tex = new STexture(renderer, window);
    tex->load("./src/Rei1.0.png");
    tex->executeShader(fragment, skin);
}
...
void Game::render()
{
    ...
    // SDL_Surface *surface = IMG_Load("./src/Rei1.0.png");
    ...
    // tex->executeShader(fragment);
    // tex->renderWithShader(100, 100, 500, 500, fragment);
    tex->render(100, 100, 500, 500);
    // Uint32 color = tex.getPixel(135, 209);
